...
import { Send, Phone, Video, MoreVertical, Sparkles, Terminal, ChevronRight, CheckCheck, Loader2, Bot, ShieldCheck, Zap, Layers } from 'lucide-react';
        ...
        <div className="w-full max-w-md bg-[#111B21] rounded-3xl border border-slate-800 shadow-2xl overflow-hidden flex flex-col h-[720px]">
            ...
            <div className="flex items-center space-x-2">
              {presetPrompts.map((p, idx) => (
                <button
                  ...
                  disabled={loading}
                  className="px-2.5 py-1 rounded-lg text-[11px] font-medium bg-slate-800 hover:bg-swiggy-orange hover:text-white text-slate-300 transition-colors duration-150 flex items-center space-x-1 border border-slate-700/60 shrink-0"
                >
          ...
          <div className="bg-[#202C33] p-3 flex items-center space-x-2 border-t border-slate-800">
            <input
              ...
              className="flex-1 bg-[#2A3942] text-slate-100 text-xs rounded-full px-4 py-2 focus:outline-none focus:ring-1 focus:ring-swiggy-orange placeholder-slate-400 font-medium"
            />
            <button
              onClick={() => handleSendMessage()}
              disabled={loading || !inputMessage.trim()}
              className="w-9 h-9 rounded-full bg-swiggy-orange hover:bg-swiggy-hover text-white flex items-center justify-center transition disabled:opacity-50 shadow shrink-0"
            >
        ...
        <div className="glass-panel p-4 rounded-2xl border border-slate-800">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="font-bold text-white text-sm">Agent Telemetry & Execution Flow</h3>
            </div>
        ...
        <div className="glass-panel p-4 rounded-2xl border border-slate-800 space-y-3 max-h-[540px] overflow-y-auto">
          ...
          {activeLogs.length === 0 ? (
            ...
            activeLogs.map((log, idx) => (
              <div key={idx} className="bg-slate-900/90 rounded-xl p-3 border border-slate-800 text-xs space-y-1.5 hover:border-slate-700 transition-colors">
                ...
                <div className="font-mono text-[10px] text-slate-300 bg-[#070A0F] p-2 rounded border border-slate-800/80 overflow-x-auto whitespace-pre-wrap">
                  {log.detail}
                </div>
              </div>
            ))
